...
Blockly.Python.init = function (a) {
	...
	if (Blockly.Python.variableDB_.setVariableMap) {
		Blockly.Python.variableDB_.setVariableMap(a.getVariableMap());
	} else {
		Blockly.Python.variableDB_.variableMap_ = a.getVariableMap();
	}
};
Blockly.Python.finish = function (a) {
	var imports = [], froms = [], definitions = [], functions = [], setups = [], card = window.localStorage.card;
	...
	let setupCode = setups.length ? setups.join('\n') + '\n\n' : '';
	return (imports.join("\n") + "\n\n" + froms.join("\n\n") + "\n\n" + definitions.join("\n") + "\n\n" + functions.join('\n') + "\n\n" + setupCode).replace(/\n\n+/g, "\n\n").replace(/\n*$/, "\n\n") + a
};
