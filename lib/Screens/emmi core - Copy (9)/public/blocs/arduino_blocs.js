...
Blockly.Blocks["digital_write_ir"] = {
    init: function () {
        var card = window.localStorage.card;

        this.setColour("#54BCF7"); // changed color
        this.setHelpUrl(Blockly.Msg.HELPURL);

        this.appendDummyInput()
            .appendField("digital write infrared") // changed visible text
            .appendField(new Blockly.FieldDropdown(profile[card].dropdownDigital), "PIN");
...
Blockly.Blocks["inout_digital_writev1"] = {
    init: function () {
        var card = window.localStorage.card;
        this.setColour("#00929f");
        this.setHelpUrl(Blockly.Msg.HELPURL);
...
Blockly.Blocks["digital_read_mic"] = {
    init: function () {
        var card = window.localStorage.card;
        ...
        this.appendDummyInput()
            .appendField("digital read mic") // changed visible text
            .appendField(new Blockly.FieldDropdown(profile[card].dropdownDigital), "PIN");
...
Blockly.Blocks["inout_analog_write_esp32"] = {
    init: function () {
        var card = window.localStorage.card;
        this.setColour("#00929f");
        this.setHelpUrl(Blockly.Msg.HELPURL);
...
Blockly.Blocks["inout_analog_touch"] = {
    init: function () {
        var card = window.localStorage.card;
        ...
        if (prog != "python") {
            ...
            this.appendDummyInput()
                .appendField("Analog read Touch A0");
        }

        this.setOutput(true, "Number");
        this.setTooltip("Reads analog touch value");
    }
};
Blockly.Blocks["analog_read_mic"] = {
    init: function () {
        var card = window.localStorage.card;
        var prog = window.localStorage.prog;
        ...
        if (prog != "python") {
            this.appendDummyInput()
                .appendField("analog read mic") // changed visible text
                .appendField(new Blockly.FieldDropdown(profile[card].dropdownAnalog), "broche");
        } else {
            this.appendDummyInput()
                .appendField("analog read mic A0");
        }
...
Blockly.Blocks["inout_analog_read2"] = {
    init: function () {
        var card = window.localStorage.card;
        var prog = window.localStorage.prog;
        this.setColour("#00929f");
...
Blockly.Blocks["inout_analog_read3"] = {
    init: function () {
        var card = window.localStorage.card;
        var prog = window.localStorage.prog;
        this.setColour("#54BCF7");
...
Blockly.Blocks["inout_attachInterrupt_esp8266"] = {
    init: function () {
        var card = window.localStorage.card;
        var prog = window.localStorage.prog;
        this.setColour("#00929f");
...
Blockly.Blocks["inout_attachInterrupt_esp32"] = {
    init: function () {
        var card = window.localStorage.card;
        this.setColour("#00929f");
        this.setHelpUrl(Blockly.Msg.HELPURL);
...
Blockly.Blocks["base_setup_loop"] = {
    ...
    onchange: function () {
        ...
        var workspace = this.workspace || Blockly.getMainWorkspace();
        if (!workspace || (workspace.isDragging && workspace.isDragging())) return;
        var count = 0;
        var blocks = workspace.getAllBlocks();
        for (var i = 0; i < blocks.length; i++) {
            if (blocks[i].type == "base_setup_loop" || blocks[i].type == "base_loop") {
                count++;
            }
        }
        if (count > 1) {
...
Blockly.Blocks["base_setup"] = {
    ...
    onchange: function () {
        ...
        var workspace = this.workspace || Blockly.getMainWorkspace();
        if (!workspace || (workspace.isDragging && workspace.isDragging())) return;
        var blocks = workspace.getBlocksByType("base_setup");
        if (blocks.length > 1) {
            this.dispose();
        }
    }
...
Blockly.Blocks["base_loop"] = {
    ...
    onchange: function () {
        ...
        var workspace = this.workspace || Blockly.getMainWorkspace();
        if (!workspace || (workspace.isDragging && workspace.isDragging())) return;
        var blocks = workspace.getBlocksByType("base_loop");
        if (blocks.length > 1) {
            this.dispose();
        }
    }
...
Blockly.Blocks["base_define"] = {
    ...
    onchange: function () {
        ...
        var workspace = this.workspace || Blockly.getMainWorkspace();
        if (!workspace || (workspace.isDragging && workspace.isDragging())) return;
        var blocks = workspace.getBlocksByType("base_define");
        if (blocks.length > 1) {
            this.dispose();
        }
    }
...
Blockly.Blocks["base_begin"] = {
    ...
    onchange: function () {
        ...
        var workspace = this.workspace || Blockly.getMainWorkspace();
        if (!workspace || (workspace.isDragging && workspace.isDragging())) return;
        var blocks = workspace.getBlocksByType("base_begin");
        if (blocks.length > 1) {
            this.dispose();
        }
    }
...
Blockly.Blocks["base_end"] = {
    ...
    onchange: function () {
        ...
        var workspace = this.workspace || Blockly.getMainWorkspace();
        if (!workspace || (workspace.isDragging && workspace.isDragging())) return;
        var blocks = workspace.getBlocksByType("base_end");
        if (blocks.length > 1) {
            this.dispose();
        }
    }
...
Blockly.Blocks["digital_mcp_write"] = {
    init: function () {
        var card = window.localStorage.card;
...
Blockly.Blocks["digital_mcp_read"] = {
    init: function () {
        var card = window.localStorage.card;
        this.setColour("#00929f");
        this.setHelpUrl(Blockly.Msg.HELPURL);
