/***************************************************************
*
*  This module was created by Oscar Ferruz. oferruz@logix5.com
*
****************************************************************/
'use strict';

goog.provide('Blockly.Blocks.ESPFunctions');
goog.provide("Blockly.Blocks.arduino");
goog.require('Blockly.Blocks');

Blockly.Blocks['ESP_yield_block'] = {
  helpUrl: '',
  init: function () {
    this.setColour("#00929f");
    this.appendDummyInput()
      .appendField(new Blockly.FieldImage("media/ESP8266icon.png", 33, 33))
      .appendField(Blockly.Msg.ESP_yield)
    this.setInputsInline(true);
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setTooltip('ESP yield');
  }
};

Blockly.Arduino['ESP_yield_block'] = function (block) {

  var code = 'yield();\n';

  return code;

};

Blockly.Java['ESP_yield_block'] = function (block) {
  var code = "ESP_yield_block();\n";
  return code;
};


Blockly.Python['ESP_yield_block'] = function (block) {
  var code = 'ESP_yield_block()\n';
  return code;
};


Blockly.Blocks['ESP_DeepSleep'] = {
  helpUrl: '',
  init: function () {
    this.setColour("#00929f");
    this.appendDummyInput()
      .appendField(new Blockly.FieldImage("media/ESP8266icon.png", 33, 33))
      .appendField(Blockly.Msg.ESP_deepsleep)
    this.appendValueInput("Value")
      .setCheck('Number');
    this.appendDummyInput()
      .appendField(Blockly.Msg.ESP_timesleep);
    this.setInputsInline(true);
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setTooltip('To sleep the ESP8266 deeply. Important: You need to connect D0(GPIO16) to Reset pin');
  }
};

Blockly.Arduino['ESP_DeepSleep'] = function (block) {

  var value = Blockly.Arduino.valueToCode(this, 'Value', Blockly.Arduino.ORDER_ATOMIC);

  if (value == "0")
    var code = 'ESP.deepSleep(0);\n';
  else
    var code = 'ESP.deepSleep(' + value + 'e6);\n';

  return code;
};

Blockly.Java['ESP_DeepSleep'] = function (block) {
  var value = Blockly.Java.valueToCode(block, 'Value', Blockly.Java.ORDER_ATOMIC) || '';
  var code = "ESP_DeepSleep(" + value + ");\n";
  return code;
};


Blockly.Python['ESP_DeepSleep'] = function (block) {
  var value = Blockly.Python.valueToCode(block, 'Value', Blockly.Python.ORDER_ATOMIC) || '';
  var code = 'ESP_DeepSleep(' + value + ')\n';
  return code;
};




Blockly.Blocks['ESP_Restart'] = {
  helpUrl: '',
  init: function () {
    this.setColour("#00929f");
    this.appendDummyInput()
      .appendField(new Blockly.FieldImage("media/ESP8266icon.png", 33, 33))
      .appendField(Blockly.Msg.ESP_restart)
    this.setInputsInline(true);
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setTooltip('Restart ESP8266 ');
  }
};

Blockly.Arduino['ESP_Restart'] = function (block) {

  var code = 'ESP.restart();\n';

  return code;
};

Blockly.Java['ESP_Restart'] = function (block) {
  var code = "ESP_Restart();\n";
  return code;
};


Blockly.Python['ESP_Restart'] = function (block) {
  var code = 'ESP_Restart()\n';
  return code;
};


Blockly.Blocks['ESP32_DeepSleep'] = {
  helpUrl: '',
  init: function () {
    this.setColour("#00929f");
    this.appendDummyInput()
      .appendField(new Blockly.FieldImage("media/ESP8266icon.png", 33, 33))
      .appendField(Blockly.Msg.ESP_deepsleep)
    this.appendValueInput("Value")
      .setCheck('Number');
    this.appendDummyInput()
      .appendField(Blockly.Msg.ESP_timesleep);
    this.setInputsInline(true);
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setTooltip('Time in second to sleep');
  }
};

Blockly.Arduino['ESP32_DeepSleep'] = function (block) {

  var value = Blockly.Arduino.valueToCode(this, 'Value', Blockly.Arduino.ORDER_ATOMIC);

  var code = 'esp_sleep_enable_timer_wakeup(' + value + 'e6);\nesp_deep_sleep_start();\n';

  return code;
};

Blockly.Java['ESP32_DeepSleep'] = function (block) {
  var value = Blockly.Java.valueToCode(block, 'Value', Blockly.Java.ORDER_ATOMIC) || '';
  var code = "ESP32_DeepSleep(" + value + ");\n";
  return code;
};


Blockly.Python['ESP32_DeepSleep'] = function (block) {
  var value = Blockly.Python.valueToCode(block, 'Value', Blockly.Python.ORDER_ATOMIC) || '';
  var code = 'ESP32_DeepSleep(' + value + ')\n';
  return code;
};

