...
class BlockCreatorApp {
    constructor() {
        ...
        // Pool of recycled block IDs available for reuse
        this.recycledBlockIds = new Set();
    ...
    reorderBlockInCategory(boardId, categoryPath, blockId, newIndex) {
        const categories = this.getBoardCategoryDefinitions(boardId);
        const category = this.findCategoryByPath(categories, categoryPath);
        if (!category || !Array.isArray(category.contents)) return;
        ...
        const oldIdx = blockItems.findIndex(item => item.type === blockId);
        if (oldIdx < 0 || oldIdx === newIndex) return;

        const [moved] = blockItems.splice(oldIdx, 1);
        blockItems.splice(newIndex, 0, moved);

        category.contents = [...otherItems, ...blockItems];
        this.saveToolboxModelToSource();
        this.renderTree();
        this.showToast('Block reordered. Press Sync to save.', 'info');
    }
    ...
    buildCategoryTree(boardId, boardBlocks) {
        ...
        // Allow referencing board-specific blocks AND common blocks
        const commonBlocks = this.blocks.filter(b => (b.boardId || this.getBoardIdForFile(b.sourceFile)) === 'common');
        [...commonBlocks, ...boardBlocks].forEach(block => {
    ...
    renderCategoryNode(boardId, node, depth = 0) {
        ...
        for (let blockIdx = 0; blockIdx < node.blocks.length; blockIdx++) {
            const block = node.blocks[blockIdx];
            ...
            blockEl.dataset.boardId = boardId;
            blockEl.dataset.categoryPath = categoryPath;
            ...
            blockEl.addEventListener('dragend', (e) => {
                ...
                document.querySelectorAll('.tree-block.drag-over-top, .tree-block.drag-over-bottom').forEach(el => {
            ...
            blockEl.addEventListener('drop', (e) => {
                ...
                try {
                    ...
                    if (data.boardId !== boardId || data.categoryPath !== categoryPath) return;
                    const rect = blockEl.getBoundingClientRect();
                    const midY = rect.top + rect.height / 2;
                    let targetIdx = blockIdx;
                    if (e.clientY >= midY) targetIdx = blockIdx + 1;
                    if (data.sourceIdx < targetIdx) targetIdx--;
                    if (data.sourceIdx !== targetIdx) {
                        this.reorderBlockInCategory(boardId, categoryPath, data.blockId, targetIdx);
                    }
                } catch (_) { }
            });
            ...
            const renameBlockBtn = document.createElement('button');
            renameBlockBtn.className = 'tree-action-btn';
            renameBlockBtn.title = 'Rename block';
            renameBlockBtn.innerHTML = '<i class="fas fa-pen"></i>';
            renameBlockBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                this.openRenameModalForBlock(block.id);
            });
            blockActions.appendChild(renameBlockBtn);
    ...
    getTemplateSuggestedId(templateKey) {
        ...
        const baseId = suggestions[templateKey] || 'new_block';

        // Prefer recycled ID that starts with the base name
        let foundRecycled = null;
        if (this.recycledBlockIds) {
            for (const recycled of this.recycledBlockIds) {
                if ((recycled === baseId || recycled.startsWith(baseId + '_'))
                    && !this.blocks.some(b => b.id === recycled)) {
                    foundRecycled = recycled;
                    break;
                }
            }
        }
        ...
        if (foundRecycled) {
            this.recycledBlockIds.delete(foundRecycled);
            return foundRecycled;
        }

        // Generate a new non-conflicting ID
        let suggestedId = baseId;
        let counter = 1;
        while (this.blocks.some(b => b.id === suggestedId)) {
            suggestedId = `${baseId}_${counter}`;
            counter++;
        }
        return suggestedId;
    }
    ...
    deleteBlockById(blockId, options = {}) {
        ...
        if (this.recycledBlockIds) {
            this.recycledBlockIds.add(block.id);
        }

        // Remove from runtime memory to prevent preview conflicts
        if (typeof Blockly !== 'undefined' && Blockly.Blocks && Blockly.Blocks[block.id]) {
            delete Blockly.Blocks[block.id];
        }
    ...
    openRenameModalForBlock(blockId) {
        this.selectedBlockId = blockId;
        const block = this.blocks.find(b => b.id === blockId);
        if (!block) return;
        this.selectedBoardId = block.boardId || this.getBoardIdForFile(block.sourceFile);
        document.getElementById('input-rename-id').value = block.id;
        this.openModal('modal-rename');
    }
    ...
    confirmRename() {
        ...
        // Replace ID in command generator code
        if (block.gen.command && block.gen.command.code) {
            block.gen.command.code = block.gen.command.code.replace(
                new RegExp(`\\['${this.escapeRegex(oldId)}'\\]`, 'g'),
                `['${newId}']`
            );
    ...
    generateFromVisual() {
        ...
        const emmiCode = this.generateEmmiFromVisual(state);
        ...
        if (document.getElementById('gen-command')) {
            document.getElementById('gen-command').value = emmiCode;
        }
        ...
        if (block) {
            ...
            block.gen.command.code = emmiCode;
        }
    ...
    generateEmmiFromVisual(state) {
        const id = state.blockId;
        const fields = this._collectFieldNames(state);
        const template = (document.getElementById('vb-emmi-template')?.value || '').trim();

        if (template) {
            let processed = template.replace(new RegExp('{BLOCK_ID}', 'g'), id);
            for (const f of fields) {
                const placeholder = `{${f.name}}`;
                // EMMI script in the visual builder uses string replacements just like the others.
                processed = processed.replace(new RegExp(placeholder.replace(/[{}]/g, '\\$&'), 'g'), f.name);
            }
            return processed;
        } else {
            return `registry['${id}'] = {\n    init: '',\n    main: ''\n};`;
